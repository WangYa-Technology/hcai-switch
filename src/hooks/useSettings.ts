    ...
    async (updates: Partial<SettingsFormState>): Promise<SaveResult | null> => {
      ...
      try {
        ...
        const {
          ...
          geminiConfigDir: _ignoredGeminiDir,
          openclawConfigDir: _ignoredOpenclawDir,
          hermesConfigDir: _ignoredHermesDir,
          ...restSettings
        } = mergedSettings;
  ...
  const saveSettings = useCallback(
    async (
      ...
      try {
        ...
        // 如果 Claude/Codex/OpenCode 的目录覆盖发生变化，则立即将"当前使用的供应商"写回对应应用的 live 配置
        ...
        if (
          !pluginSynced &&
          (claudeDirChanged || codexDirChanged || opencodeDirChanged)
        ) {
          const syncResult = await syncCurrentProvidersLiveSafe();
          if (!syncResult.ok) {
            console.warn(
